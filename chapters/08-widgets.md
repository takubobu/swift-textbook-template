...
**実験1：**
- やったこと：placeholder 関数内の返り値を Quote(id: 0, text: "★★★★★★★★", author: "データ読み込み中") のように伏字や仮のテキストに変更してみる。
- 結果：ウィジェットを読み込んでいる最中や初期化時に、設定したダミーテキストが薄く表示される動きを確認できた。
- わかったこと：placeholder はシステムがデータ準備中のスケルトン表示（枠組み）として使うため、ユーザーに不快感を与えない「読み込み中UI」を用意する重要性を学んだ。

**実験2：**
- やったこと：smallWidget 内の Text(entry.quote.text) についている .lineLimit(3) をコメントアウトし、あえて長い名言を表示させてみる。
- 結果：枠に入りきらなくなったテキストが途中で切れ、下にある作者名（author）がウィジェットの外にはみ出して見えなくなってしまった。
- わかったこと：スペースが厳密に決まっているウィジェットUIでは、.lineLimit や .minimumScaleFactor（文字サイズの自動縮小）を使って「枠内に収める防御的デザイン」が必須であると実感した。

## AIに聞いて特に理解が深まった質問 TOP3

1. **質問：**
   なぜ QuoteStore.swift の Target Membership（両方へチェック）を設定する必要があるの？
   
   **得られた理解：**
   メインアプリとウィジェット（Extension）は完全に「別のプログラム」として実行されるため。両方のプログラムにデータ定義を含めないと「そんな型は見つかりません（Cannot find in scope）」というエラーになる。

2. **質問：**
   TimelineProvider の placeholder と getSnapshot の違いは？
   
   **得られた理解：**
   placeholder はデータ読み込み中に表示される「骨組み（グレーの影など）」、getSnapshot はウィジェットを追加するとき（ギャラリー画面）で見せる「サンプル画像」の役割を持っている。

3. **質問：**
   なぜ @main がついたファイルを削除しないと「'main' attribute can only apply to one type」エラーになるの？
   
   **得られた理解：**
   Swiftのエクステンション（Widget Extension）の中にプログラムの実行開始点（@main）が2つ以上存在してしまうため。不要な Bundle ファイルを消して @main を1つだけに絞る必要がある。

## この章のまとめ

WidgetKitを活用し、アプリ本体と独立してホーム画面上で動く「名言ウィジェット」の実装方法を学んだ。

・ターゲット間のコード共有： Target Membership を適切に設定し、データ型（Quote / QuoteStore）を双方のプログラムで安全に共有する手順を理解した。

・タイムライン更新管理： TimelineProvider を使い、指定した時刻（翌日0時）に非同期でウィジェットを自動更新するスケジューリングの仕組みを習得した。

・サイズに応じたUI制御： @Environment(\.widgetFamily) でウィジェットサイズ（small / medium）を判定し、表示枠に収める適切なレイアウト設計の重要性を学んだ。
