...
この章では、アプリの設定値を保存する AppStorage と、メモなどの構造化されたデータを保存・管理する SwiftData の使い方を理解する。また、保存したデータの追加・編集・削除・表示を実装し、アプリを終了してもデータが保持される仕組みを学習する。さらに、NavigationStackやList、Formなどを利用したメモアプリの基本的な画面構成についても学ぶ。
...
@Model マクロを付与した Memo クラスを定義し、永続化の対象となるデータ構造を定義している。

**なぜこう書くのか：**

SwiftDataではクラスに @Model を付けるだけで、バックエンドのデータベースとのやり取りやテーブル構造の生成、変更の追跡などをフレームワークが自動で行ってくれる。

**もしこう書かなかったら：**

@Model を付けない通常のクラスや構造体（struct）にすると、SwiftDataはこれをデータベース管理対象として認識できず、データの永続化機能を利用できなくなってしまう。
...
UserDefaults への読み書きをプロパティ感覚で行える @AppStorage を使用している。
そして、指定したキーの値が更新されると自動で保存され、連動する画面も自動で再描画される。

**なぜこう書くのか：**

ユーザー名やスイッチの状態など、検索が不要な単一の設定値を保存するのに最も効率的だから。
また、手動の保存・監視処理を書く必要がなく、SwiftUIの画面更新とスムーズに同期できるため。

**もしこう書かなかったら：**

UserDefaults へ手動で保存する処理や、画面を更新するためのデータ監視コードが必要になる。
また、設定保存のために SwiftData モデルを作成することになり、コードが複雑化してしまう。
