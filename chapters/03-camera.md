...
**実験1：**
- やったこと：CameraView の capturedImage: Binding<UIImage?> を、通常の変数 var capturedImage: UIImage? に変更してみた。
- 結果：カメラで撮影して戻ってきても、ContentView 側の画像表示エリアが更新されず、画像が反映されなかった。
- わかったこと：@Binding は親と子の間でデータを同期するので、これがないと子ビューで行った変更が親ビューに伝わらないことがわかった。

**実験2：**
- やったこと：.frame(maxHeight: 400) をコメントアウトして実行した。
- 結果：スマホのカメラで撮った高解像度の画像が、画面いっぱいに表示されてしまい、画面をはみ出した。
- わかったこと：.frame(maxHeight: 400) は単なる指定ではなく、ユーザーのスマホ環境に関わらず、画面内に画像を収めるために必須の機能であることがわかった。

## AIに聞いて特に理解が深まった質問 TOP3

1. **質問：**　なぜカメラの実装には UIViewControllerRepresentable が必要なのですか？

   **得られた理解：**　カメラ機能を担う UIImagePickerController はUIKitのコンポーネントだから。SwiftUI側からこのクラスを直接扱うためのラップが必要であり、その役割を果たすのが UIViewControllerRepresentable プロトコル。

2. **質問：**　onChange で画像データを受け取る際になぜ Task を使うのですか？

   **得られた理解：**　item.loadTransferable はネットワークやディスクからの読み込みを伴う非同期関数であるため、メインスレッドを止めずにバックグラウンドで安全に処理を行うために、Task ブロック内で実行する必要がある。

3. **質問：**　Coordinator は具体的に何をしているのですか？

   **得られた理解：**　UIKitのカメラから送られてくる「写真が撮れたよ！」というイベントを、SwiftUI側で受け取れるように変換する通訳みたいなもの。Coordinator が通知係として間に立つことで、初めてUIKitからSwiftUIへデータを橋渡しできることがわかった。

## この章のまとめ

SwiftUIとUIKitを橋渡しする仕組みの UIViewControllerRepresentable と Coordinator の理解と、非同期処理（Task）や状態管理（@State, @Binding）を活用した、実機連携アプリの構築手法を学びました。
